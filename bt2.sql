...
WHERE
    PX.NgayBan BETWEEN '2024-02-14' AND '2024-05-16'
GROUP BY
    PX.SoPX, NV.HoTen, PX.NgayBan;
...
GROUP BY
    PX.SoPX, PX.NgayBan, PX.MaKH, KH.TenKH;
...
SELECT
    MONTH(PX.NgayBan) AS month,
    SUM(CTPX.GiaBan * CTPX.SoLuong) AS DoanhThu
FROM
    CTPhieuXuat AS CTPX
        JOIN PhieuXuat PX ON CTPX.SoPX = PX.SoPX
WHERE
    MONTH(PX.NgayBan) BETWEEN 1 AND 6
GROUP BY
    MONTH(PX.NgayBan)
ORDER BY
    MONTH(PX.NgayBan);
...
WHERE
    MONTH(PX.NgayBan) BETWEEN 4 AND 6
GROUP BY
    CTPX.SoPX, NV.HoTen, KH.TenKH;
...
WHERE
    MONTH(PX.NgayBan) = MONTH(CURRENT_DATE())
GROUP BY
    PX.SoPX, KH.MaKH, NV.HoTen, PX.NgayBan;
...
FROM
    ...
    NhanVien NV ON PX.MaNV = NV.MaNV
GROUP BY
    PX.MaNV, NV.HoTen, SP.MaSP, SP.TenSP, SP.DonViTinh;
...
WHERE
  ...
  AND KH.MaKH = 'KH1'
GROUP BY
    PX.SoPX, PX.NgayBan, SP.MaSP, SP.TenSP, CPX.SoLuong, SP.DonViTinh;
...
FROM
    ...
    CTPhieuXuat CPX ON SP.MaSP = CPX.MaSP
        JOIN
    PhieuXuat PX ON CPX.SoPX = PX.SoPX
WHERE
    MONTH(PX.NgayBan) BETWEEN 1 AND 6
...
FROM NhaCungCap NCC
    ...
    FROM PhieuNhap
    WHERE YEAR(NgayNhap) = 2018 AND MONTH(NgayNhap) BETWEEN 4 AND 6
    UNION
    SELECT DISTINCT MaKH
    FROM PhieuXuat
    WHERE YEAR(NgayBan) = 2018 AND MONTH(NgayBan) BETWEEN 4 AND 6
) AS GD
...
GROUP BY
    KH.MaKH
HAVING
    SUM(CTPX.GiaBan * CTPX.SoLuong) = (
        SELECT
            MAX(Total)
        FROM
            (
                SELECT
                    SUM(CTPX.GiaBan * CTPX.SoLuong) AS Total
                FROM
                    PhieuXuat PX
                        JOIN
...
GROUP BY
    KH.MaKH, KH.TenKH
HAVING
    SUM(CPX.GiaBan * CPX.SoLuong) = (
        SELECT
            MAX(Total)
        FROM
            (
                SELECT
                    SUM(CPX.GiaBan * CPX.SoLuong) AS Total
                FROM
                    KhachHang KH
                        JOIN
                    PhieuXuat PX ON KH.MaKH = PX.MaKH
